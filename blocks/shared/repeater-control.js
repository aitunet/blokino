/* ==========================================================================
 * BloqUIX · UI de repeater COMPARTIDA (editor)
 * --------------------------------------------------------------------------
 * `window.bloquix.RepeaterControl` — acordeón de items en el InspectorControls:
 ...
 *
 * `window.bloquix.MediaField` — campo de imagen/avatar NATIVO (MediaUpload de WP)
...
( function ( wp ) {
	 ...
	 * ------------------------------------------------------------------- */
	window.bloquix.MediaField = function ( props ) {
		var id = props.id || 0;
		var url = props.url || '';
		var setLabel = props.setLabel || __( 'Set image', 'bloquix' );
		var replaceLabel = props.replaceLabel || __( 'Replace image', 'bloquix' );
		var cls = 'bloquix-media-field' + ( props.round ? ' is-round' : '' );
				...
				render: function ( o ) {
					return el(
						...
						url
							? el(
								'button',
								{ type: 'button', className: 'bloquix-media-field__preview', onClick: o.open, 'aria-label': replaceLabel },
								el( 'img', { src: url, alt: '' } )
							)
							: el(
								'button',
								{ type: 'button', className: 'bloquix-media-field__placeholder', onClick: o.open },
								setLabel
							),
						el(
							'div',
							{ className: 'bloquix-media-field__actions' },
							el( C.Button, { variant: 'secondary', size: 'small', onClick: o.open }, url ? replaceLabel : setLabel ),
							( id || url )
								? el( C.Button, { variant: 'tertiary', size: 'small', isDestructive: true, onClick: props.onRemove }, __( 'Remove', 'bloquix' ) )
								: null
						)
					);
				}
	 ...
	 * ------------------------------------------------------------------- */
	window.bloquix.RepeaterControl = function ( props ) {
		...
		var max = props.max || 0;
		var addLabel = props.addLabel || __( 'Add item', 'bloquix' );
		var itemLabel = props.itemLabel || function ( it, i ) {
			return __( 'Item', 'bloquix' ) + ' ' + ( i + 1 );
		};
		...
		var panels = items.map( function ( item, index ) {
			...
			return el(
				...
				el(
					'div',
					{ className: 'bloquix-repeater__item', onMouseDownCapture: activate, onFocusCapture: activate },
					renderItem( item, index, function ( patch ) { update( index, patch ); } ),
					el(
						'div',
						{ className: 'bloquix-repeater__actions', style: { display: 'flex', gap: '4px', marginTop: '12px' } },
						el( C.Button, { variant: 'secondary', size: 'small', disabled: index === 0, onClick: function () { move( index, -1 ); }, label: __( 'Move up', 'bloquix' ) }, '↑' ),
						el( C.Button, { variant: 'secondary', size: 'small', disabled: index === items.length - 1, onClick: function () { move( index, 1 ); }, label: __( 'Move down', 'bloquix' ) }, '↓' ),
						el( C.Button, { variant: 'secondary', isDestructive: true, size: 'small', onClick: function () { remove( index ); } }, __( 'Remove', 'bloquix' ) )
					)
				)
			);
		} );
