...
- **Effects on native blocks (`tf*` attributes).** A *Blokino Effects* panel on Group, Columns, Cover, Image, Heading, Paragraph and Buttons: entrance animations (fade-up, clip-reveal, mask-up, blur-in, scale-in, slide, text-stagger), hover effects (lift, glow, tilt, magnetic, image-zoom, underline-grow), scroll effects (parallax, sticky-pin, reveal, progress), blend modes and animated borders. A block with no effect attribute stays completely clean.
...
- **A free theme to start with.** [Blokmark](https://www.tunetdesign.com/downloads/blokmark/) is built for this engine and free; the Themes screen lists it first, then the premium themes.
...
Download the latest release ZIP (or clone this repository into `wp-content/plugins/blokino`) and activate **Blokino** from *Plugins*. No build step is required to run the plugin: the shipped JavaScript and CSS are plain, unbundled files.
...
Conventions: PHP prefix `blokino_` / `Blokino_`, CSS custom properties `--tnt-`, block namespace `blokino/`, data attributes `data-tf-`, text domain `blokino`. Every public capability must be additive and backward compatible; themes already built on the engine must keep working after any change.

Translations live in `languages/` (`blokino.pot` + compiled `.mo`/`.json` per locale).
...
Issues and pull requests are welcome at <https://github.com/aitunet/blokino>. Please keep changes additive, respect the conventions above and run the linters before submitting.
